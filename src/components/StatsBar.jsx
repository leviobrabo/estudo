...
export default function StatsBar({ r }) {
  ...
  return (
    <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
      {cards.map((c) => (
        <div key={c.rotulo} className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 shadow-sm">
          <div className="text-[11px] uppercase tracking-wide text-gray-400">{c.rotulo}</div>
          <div className={`mt-0.5 text-xl font-semibold ${c.destaque ? 'text-blue-600' : 'text-gray-800'}`}>
            {c.valor}
          </div>
        </div>
      ))}
